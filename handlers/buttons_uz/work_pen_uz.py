...
from data.config import C_USER, C_PASS, C_URI, C_INPS, PINFL
...
@dp.message_handler(text='Буюртмани бекор қилиш ❌',
                    state=[work_pen_uz.fio, work_pen_uz.dates, work_pen_uz.orders, work_pen_uz.year, work_pen_uz.months,
                           work_pen_uz.dayru,
                           work_pen_uz.paytype])
...
@dp.message_handler(IsPrivate(), text='\"Узметкомбинат\"АЖ пенсионери')
async def work_pen_uzed(message: Message, state: FSMContext):  # Создаём асинхронную функцию
    ...
    await work_pen_uz.fio.set()


@dp.message_handler(IsPrivate(), state=work_pen_uz.fio)
async def set_fio(message: Message, state: FSMContext):
    ...
    # if True:
        ...
        if len(str(pinfl)) == 14:
            ...
            r = requests.get(C_INPS + message.text, auth=auth)
            pen_name = r.text.encode('ISO-8859-1').decode('utf-8-sig')
            if pen_name != '':
                fio = message.text
                await state.update_data(fio=pen_name)
                if len(pen_name) > 5:
                    await message.answer(f"{pen_name} телефон рақамингизни юборинг! ⬇️", reply_markup=conatct_uz)
                    await work_pen_uz.tel.set()
    except:
        pass


@dp.message_handler(IsPrivate(), content_types=ContentType.CONTACT, state=work_pen_uz.tel)
async def set_fio(message: Message, state: FSMContext):
    ...
    if len(tel) > 7:
        ...
        await work_pen_uz.orders.set()
    else:
        await message.answer(f"{fio} телефон рақамингизни юборинг! ⬇️", reply_markup=conatct_uz)
        await work_pen_uz.tel.set()


@dp.message_handler(IsPrivate(), state=work_pen_uz.orders)
async def set_fio(message: Message, state: FSMContext):
    try:
        data = await state.get_data()
        fio = data.get('fio')
        ...
        await commands.add_order(message.from_user.id, fio, '-----', False, tel, pension_30=30)
        if quantity_orders:
            try:
...
@dp.message_handler(IsPrivate(), state=work_pen_uz.year)
async def set_fio(message: Message, state: FSMContext):
    try:
        year_now = date.today()
        ...
        await work_pen_uz.months.set()
    except:
        await message.answer('Буюртма бекор қилинди. Хатолик юз берди!', reply_markup=lang_btn)
        await state.finish()


@dp.message_handler(IsPrivate(), state=work_pen_uz.months)
async def set_fio(message: Message, state: FSMContext):
    try:
        ...
        if list3 == []:
            ...
            await work_pen_uz.dayru.set()
    except:
        await message.answer('Буюртма бекор қилинди. Хатолик юз берди!', reply_markup=lang_btn)
        await state.finish()


@dp.message_handler(IsPrivate(), state=work_pen_uz.dayru)
async def set_day(message: Message, state: FSMContext):
    ...
    try:
        ...
        if int(text) in days_x:
            ...
            await work_pen_uz.cat_room.set()
...
@dp.message_handler(IsPrivate(), state=work_pen_uz.cat_room)
async def set_day(message: Message, state: FSMContext):
    try:
        # if True:
        ...
        try:
            ...
            try:
                ...
                lgots += select_user_orders[0].pension_30
...
@dp.message_handler(IsPrivate(), state=work_pen_uz.paytype)
async def set_fio(message: Message, state: FSMContext):
    try:
