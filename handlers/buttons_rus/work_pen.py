...
@dp.message_handler(text='Отменить заказ ❌',
                    state=[work_pen.fio, work_pen.dates, work_pen.orders, work_pen.year, work_pen.months,
                           work_pen.dayru,
                           work_pen.paytype])
...
@dp.message_handler(IsPrivate(), text='Пенсионер АО "Узметкомбинат"')
async def work_pened(message: Message, state: FSMContext):  # Создаём асинхронную функцию
    text = 'Напишите ПИНФЛ в паспорте пенсионера!'
    await message.answer_photo(PINFL)
    await message.answer(text)
    await work_pen.fio.set()


@dp.message_handler(IsPrivate(), state=work_pen.fio)
    ...
    try:
        ...
        if len(str(pinfl)) == 14:
            ...
            if pen_name != '':
                ...
                if len(pen_name) > 5:
                    await message.answer(f"{pen_name} Отправьте свой номер! ⬇️", reply_markup=conatct_ru)
                    await work_pen.tel.set()
    except:
        pass


@dp.message_handler(IsPrivate(), content_types=ContentType.CONTACT, state=work_pen.tel)
async def set_fio(message: Message, state: FSMContext):
    ...
    if len(tel) > 7:
        ...
        await work_pen.orders.set()
    else:
        await message.answer(f"{fio} Отправьте свой номер! ⬇️", reply_markup=conatct_ru)
        await work_pen.tel.set()


@dp.message_handler(IsPrivate(), state=work_pen.orders)
async def set_fio(message: Message, state: FSMContext):
    try:
        data = await state.get_data()
        fio = data.get('fio')
...
@dp.message_handler(IsPrivate(), state=work_pen.year)
async def set_fio(message: Message, state: FSMContext):
    try:
        year_now = date.today()
        ...
        await work_pen.months.set()
    except:
        await message.answer('Заказ отменен. Произошла ошибка!', reply_markup=lang_btn)
        await state.finish()


@dp.message_handler(IsPrivate(), state=work_pen.months)
async def set_fio(message: Message, state: FSMContext):
    try:
        ...
        if list3 == []:
            ...
            await work_pen.dayru.set()
    except:
        await message.answer('Заказ отменен. Произошла ошибка!', reply_markup=lang_btn)
        await state.finish()


@dp.message_handler(IsPrivate(), state=work_pen.dayru)
...
@dp.message_handler(IsPrivate(), state=work_pen.cat_room)
async def set_day(message: Message, state: FSMContext):
    try:
...
@dp.message_handler(IsPrivate(), state=work_pen.paytype)
async def set_fio(message: Message, state: FSMContext):
    try:
